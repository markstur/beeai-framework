...
<Note>
Explore the implementation in [Python](https://github.com/i-am-bee/beeai-framework/tree/main/python/beeai_framework/agents) or
[TypeScript](https://github.com/i-am-bee/beeai-framework/tree/main/typescript/src/agents).
</Note>
...
The RemoteAgent is a powerful client-side abstraction designed to facilitate communication with agents running on remote servers, containers, or cloud environments. It provides an asynchronous interface for sending prompts, receiving responses, and managing agent state.

Use RemoteAgent with the framework’s workflow component to build multi-agent systems that:
- Connect to multiple agents, each hosted at a different API URL
- Coordinate all agents within a single application
- Add or remove agents easily, thanks to a modular design
- Work across different technologies, even if agents are built with different frameworks or run in different environments

<Tip>
Before running a RemoteAgent, ensure the agent server is running at the specified URL.
</Tip>
