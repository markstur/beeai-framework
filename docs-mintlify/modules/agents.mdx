...
An AI agent is a sophisticated system built on top of large language models (LLMs) that can solve complex problems through structured reasoning and autonomous actions. Unlike simple chatbots that only respond to individual prompts, agents are able to:
...
You may choose the appropriate memory strategy based on your use case:
- UnconstrainedMemory: Unlimited message storage
- SlidingMemory: Fixed-size message window
- TokenMemory: Token-count based memory management
- SummarizeMemory: Summarizes conversation history
