...
The RemoteAgent is a client-side abstraction for communicating with remote agents over API endpoints. It provides an asynchronous interface for sending prompts, receiving responses, and managing agent state. This allows agents to run on remote servers, containers, or cloud environments, rather than locally.
